...
%-------------------------------------------------- 
\begin{abstract}
\comment{ (Page 1, top) Here we summarize the contribution of this work.
...
\comment{ Our argument for sentence topic model goes here...}.

\comment{ NOTE: We need to address the difference between our model, hLDA and
Pachinko Allocation Model (PAM).  The most difficult one is ``latent-Dirichlet
co-clustering'', which is almost identical to our work. }
...
\comment{ Putting so much math in here is not a good idea.  Replace it with
...
\comment{ (One page) } The performance of the LDA-based model can largely rely
...
The experimental result is shown in Figure~\ref{f:perplexity}.
\comment{(Describe the results)}
...
% Other stuff
%-------------------------------------------------- 
\section{Related Work} \label{s:related-work} \comment{ (Page 7, Half a page) Mostly topic models. }  

\section{Discussions} \label{s:discussions} \comment{ (Page 8, 3/4 page)  } 
...
\section{Concluding Remarks} \label{s:concluding-remarks} \comment{ (Page 8, 1/4 page) }

\comment{NOTE: If I figured out what to write in the abstract/end of
introduction, I'll have no problem fill this in. }

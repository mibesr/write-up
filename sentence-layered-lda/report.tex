...
%--------------------------------------------------
% that transforms a pseudo (or
% {\it prototypical}) document, composed of latent semantics, to a collection of words
% relies on two distributions: 
%-------------------------------------------------- 
...
% introduce a generative process and the corresponding inference procedure to
% leverage the intra-document syntactical information.  Estimation for model
% parameters is described in Section~\ref{s:parameter-estimation}.  Later in
% section~\ref{s:perplexity-evaluation}, we introduce the experiments and
% describe the experimental results.  We summarize the previous efforts in
% Section~\ref{s:related-work}.  Finally, we discuss the issues discovered in
% this work and stake over concluding remarks in Section~\ref{s:discussion}.
%-------------------------------------------------- 

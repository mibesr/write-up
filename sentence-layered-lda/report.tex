...
\comment{ 
  Notation description goes here.
  Notation description goes here.
  Notation description goes here.
  Notation description goes here.
  Notation description goes here.
...
We decided to learn the parameters on a larger, multi-topic corpus.  The
dataset we used comes from the daily Spinn3r blog stream\footnote{\tt http://spinn3r.com}, for that we found the
extracts we had seemed to fit the purpose well.  \comment{Describe the size of
the dataset}.  First, we divided the data into training, test, and development
...
\comment{FIXME: As we argued in the previous sections}, sentence topics should be viewed as
...
\comment{ (Page 7, Half a page) Mostly topic models. 
NOTE: We need to address the difference between our model, hLDA and

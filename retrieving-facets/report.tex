...
An intuitive solution for facet ranking is to consider the relevance of the
documents that covers the facet: The facet discovered in a highly-relevant
document should be assigned a higher weight, and the facet found in documents
that get low relevance scores should be ranked lower accordingly.  Our idea is
to model the contribution of relevance from document to facet by extending the
existing retrieval model.  Since each facet discovered in the query session
might have a number of source documents contributing different degrees of
importance, a natural way to integrate these data is through a weighted sum.

One difficulty for deriving such an model lies in the integration of the
document relevance score.  We find it more appealing to apply this idea on a
language-model-based backend, for that ... \comment{FIXME}

...
Intuitively, faceted search can be seen as a two-phase extension of the
ordinary retrieval task: In the first phase, one populates a set of document
...
result set, or in fewer cases in lexicographical order.  We refer this type of
presentation scheme as \emph{count-based}.  The count-based method has at least
three advantages: being fast, being easy to implement, and being
straightforward so that users do not get confused.  While being as widely
accepted across applications, the count-based method suffers from its lack of
interpretability to the the \emph{relevance} of the discovered facets.  This
may lead to a bizzare scenario that, in a system where documents are retrieved
in terms of relevance, the facets discovered in the collection could only be
reported based on the number of counts.
